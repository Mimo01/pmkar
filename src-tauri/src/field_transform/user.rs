...
impl UserResolver {
    ...
    pub async fn fetch_users_by_domain(&self, domain: &str) -> Result<Vec<serde_json::Value>, ()> {
    ...
    pub async fn fetch_users_by_query(&self, query: &str) -> Result<Vec<serde_json::Value>, ()> {
        let trimmed = self.cloud_base_url.trim_end_matches('/');
        let encoded_query = urlencoding::encode(query);
        let url = format!(

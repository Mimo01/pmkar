...
// prewarmedIssueTypes is an empty map so IssueTypeChooser renders safely when
// targetProjectKey is non-empty (tests that use WR-03 targetProjectKey guard).
...
describe('CopyPreviewModal', () => {
  ...
  it('Confirm button invokes confirmCopy with connection URLs when issue type and project are set (COPY-01)', () => {
    // WR-02: isCopyDisabled gates on !targetIssueTypeId — must have an issue type set.
    // WR-03: isCopyDisabled also gates on !targetProjectKey (isProjectMissing guard added in
    // fix(24) commit 5fb7594) — targetProjectKey must be non-empty to enable the button.
    // prewarmedIssueTypes is {} in the schemaCacheStore mock so IssueTypeChooser renders
    // safely (empty list) without crashing on undefined map access.
    currentStoreState = buildStoreState({ targetIssueTypeId: 'it-1', targetProjectKey: 'MYPROJ' });
    render(<CopyPreviewModal />);
    ...
    // Phase 24 (feat 114180a): confirmCopy now receives a 3rd arg — the previewCopyId UUID
    // generated by the useEffect when phase transitions to 'previewing'. Use expect.any(String)
    // because the exact UUID is non-deterministic.
  ...
  it('Confirm button is disabled when targetProjectKey is empty (WR-03)', () => {
    // fix(24) 5fb7594: isProjectMissing = !targetProjectKey disables the Copy button
    // when no target project is configured, matching the guard in CopyPreviewPage.
    currentStoreState = buildStoreState({ targetIssueTypeId: 'it-1', targetProjectKey: '' });
    render(<CopyPreviewModal />);

...
export function FieldMappingSection() {
  ...
  async function handleAddRow() {
    // Minimalist approach: prompt for source field id (window.prompt).
    // A future polish can replace this with a combobox from source fields.
    ...
    const newRow: FieldMappingRow = {
      ...
      targetFieldId: '',
      transformerKind: 'identity',
      sourceSchema: { type: 'any' } as FieldSchemaType,
      targetSchema: { type: 'any' } as FieldSchemaType,
    };
    try {
  ...
  // Edge case: no prewarmed issue types (Pitfall 4)
  if (!firstIssueTypeId) {
    return (
